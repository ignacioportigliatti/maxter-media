...
export async function POST(request: Request) {
  ...
  try {
    ...
    const integerExpirationDays = parseInt(formData.expirationDays);

    expires.setDate(expires.getDate() + integerExpirationDays);
    ...
    for (let i = 0; i < formData.quantity; i++) {
      ...
      }-${selectedGroup.name.slice(1, 5)}-${randomBytes(2)
        .toString("hex")
        .toUpperCase()}`.toUpperCase();
      const qrCode = await generateQrCode(
        `https://${appDomain}/client?code=${formattedCode}`
      );
